            ...
            while (startDatep.isBefore(endDatep)) {
                ...
                if (users.getBranch() != null) {
                    totalInvoices = postingsRepository.findTotalNumberInvoices(users.getOutlet().getId(),
                            users.getBranch().getId(), startMonthDate, endMonthDate, 10L, true, "DR");
                    ...
                    totalInvoices = postingsRepository.findTotalNumberInvoicesNoBranch(
                            users.getOutlet().getId(), startMonthDate, endMonthDate, 10L, true, "DR");
...
//            LocalDate endDate = LocalDate.parse(request.getParameter("end_date"));
            ...
            for (LedgerTransactionPostings ledgerTransactionPostings : mlist) {
                ...
                if (users.getBranch() != null) {
                    tranxJournalDetails = tranxJournalDetailsRepository.
                            findByTranxJournalMasterIdAndOutletIdAndBranchIdAndStatusAndType(
                    ...
                    tranxJournalDetails = tranxJournalDetailsRepository.
                            findByTranxJournalMasterIdAndOutletIdAndStatusAndType(
                ...
                inside.addProperty("id", ledgerTransactionPostings.getTransactionId());
                innerArr.add(inside);
            }
    ...
    public InputStream exportToExcelJournal(Map<String, String> jsonRequest, HttpServletRequest request) throws IOException {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        try {
...
//                        System.out.println("data ------> " + s);
    ...
    public InputStream exportToExcelJournalmonth(Map<String, String> jsonRequest, HttpServletRequest request) throws IOException {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        try {
//            Boolean mfgShow = Boolean.valueOf(request.getParameter("mfgShow"));
            String JsonToStr = jsonRequest.get("list");
