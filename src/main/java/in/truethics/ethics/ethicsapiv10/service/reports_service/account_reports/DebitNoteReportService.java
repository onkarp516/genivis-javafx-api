            ...
            while (startDatep.isBefore(endDatep)) {
                ...
                if (users.getBranch() != null) {
                    totalInvoices = postingsRepository.findTotalNumberInvoices(users.getOutlet().getId(),
                            users.getBranch().getId(), startMonthDate, endMonthDate, 7L, true, "DR");
                    ...
                    totalInvoices = postingsRepository.findTotalNumberInvoicesNoBranch(
                            users.getOutlet().getId(), startMonthDate, endMonthDate, 7L, true, "DR");
...
//                    System.out.println("end Date:" + obj[1].toString());
            ...
            for (LedgerTransactionPostings ledgerTransactionPostings : mlist) {
                ...
                if (users.getBranch() != null) {
                    tranxDebitNoteDetails = tranxDebitNoteDetailsRepository.
                            findByTranxDebitNoteMasterIdAndOutletIdAndBranchIdAndStatusAndType(
                    ...
                    tranxDebitNoteDetails = tranxDebitNoteDetailsRepository.
                            findByTranxDebitNoteMasterIdAndOutletIdAndStatusAndType(
    ...
    public InputStream exportToExcelDebitNote(Map<String, String> jsonRequest, HttpServletRequest request) throws IOException {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        try {
            ...
            if (productBatchNos.size() > 0) {
                try (Workbook workbook = new XSSFWorkbook()) {
                    ...
                    Sheet sheet = workbook.createSheet("DebitNote");
...
//                        System.out.println("data ------> " + s);
    ...
    public InputStream exportToExcelDebitNotemonth(Map<String, String> jsonRequest, HttpServletRequest request) throws IOException {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        try {
            String JsonToStr = jsonRequest.get("list");
            JsonArray productBatchNos = new JsonParser().parse(JsonToStr).getAsJsonArray();
