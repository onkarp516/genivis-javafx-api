...
import static in.truethics.ethics.ethicsapiv10.service.reports_service.account_reports.PurRegisterReportService.stockLogger;
            ...
            while (startDatep.isBefore(endDatep)) {
                ...
                if (users.getBranch() != null) {
                    totalInvoices = postingsRepository.findTotalNumberInvoices(users.getOutlet().getId(),
                            users.getBranch().getId(), startMonthDate, endMonthDate, 8L, true, "DR");
                    ...
                    totalInvoices = postingsRepository.findTotalNumberInvoicesNoBranch(
                            users.getOutlet().getId(), startMonthDate, endMonthDate, 8L, true, "DR");
...
//            LocalDate endDate = LocalDate.parse(request.getParameter("end_date"));
            ...
            for (LedgerTransactionPostings ledgerTransactionPostings : mlist) {
                ...
                if (users.getBranch() != null) {
                    tranxCreditNoteDetails = tranxCreditNoteDetailsRepository.
                            findByTranxDetailsWithBR(ledgerTransactionPostings.getTransactionId(), users.getOutlet().getId(),
                    ...
                    tranxCreditNoteDetails = tranxCreditNoteDetailsRepository.
                            findByTranxDetailsNoBR(ledgerTransactionPostings.getTransactionId(), users.getOutlet().getId(),
                                    true, "DR");
                    LedgerMaster mLedger = ledgerMasterRepository.findByIdAndStatus(
                            tranxCreditNoteDetails.getLedgerMasterId(), true);
                    inside.addProperty("particulars", mLedger != null ? mLedger.getLedgerName() : "");
    ...
    public InputStream exportToExcelCreditNote(Map<String, String> jsonRequest, HttpServletRequest request) throws IOException {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        try {
...
//                        headers = new String[]{"DATE", "LEDGER NAME", "VOUCHER TYPE", "VOUCHER NO.", "DEBIT", "CREDIT"};
                    Sheet sheet = workbook.createSheet("CreditNote");
...
//                        System.out.println("data ------> " + s);
    ...
    public InputStream exportToExcelCreditNotemonth(Map<String, String> jsonRequest, HttpServletRequest request) throws IOException {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        try {
//            Boolean mfgShow = Boolean.valueOf(request.getParameter("mfgShow"));
            String JsonToStr = jsonRequest.get("list");
