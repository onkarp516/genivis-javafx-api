package in.truethics.ethics.ethicsapiv10.service.tranx_service.gstinput;
...
import in.truethics.ethics.ethicsapiv10.repository.tranx_repository.gstinput_repository.GstInputDetailsRepository;
import in.truethics.ethics.ethicsapiv10.repository.tranx_repository.gstinput_repository.GstInputDutiesTaxesRepository;
import in.truethics.ethics.ethicsapiv10.repository.tranx_repository.gstinput_repository.GstInputMasterRepository;
...
public class TranxGstInputService {
    ...
    public JsonObject gstInputLastRecord(HttpServletRequest request) {
        ...
        if (users.getBranch() != null) {
            count = gstInputMasterRepository.findBranchLastRecord(users.getOutlet().getId(), users.getBranch().getId());
        } else {
            count = gstInputMasterRepository.findLastRecord(users.getOutlet().getId());
        }
    ...
    public JsonObject createGstInput(HttpServletRequest request) {
        ...
        try {
            ...
            gstInputMaster.setOutlet(outlet);
            ...
            if (users.getBranch() != null)
                roundoff = ledgerMasterRepository.findByOutletIdAndBranchIdAndLedgerNameIgnoreCase(
                        users.getOutlet().getId(), users.getBranch().getId(), "Round off");
            else
                roundoff = ledgerMasterRepository.findByOutletIdAndLedgerNameIgnoreCaseAndBranchIsNull(
                        users.getOutlet().getId(), "Round off");
            gstInputMaster.setRoundOff(Double.parseDouble(request.getParameter("round_off_amt")));//0.52
            ...
            if (cgstList != null && cgstList.size() > 0) {
                for (JsonElement taxElement : cgstList) {
                    ...
                    if (tranxGstInput.getBranch() != null)
                        dutiesTaxes = ledgerMasterRepository.findByOutletIdAndBranchIdAndLedgerNameIgnoreCase(
                                tranxGstInput.getOutlet().getId(), tranxGstInput.getBranch().getId(), ledgerName);
                    else
                        dutiesTaxes = ledgerMasterRepository.findByOutletIdAndLedgerNameIgnoreCaseAndBranchIsNull(
                                tranxGstInput.getOutlet().getId(), ledgerName);
                    if (dutiesTaxes != null) {
