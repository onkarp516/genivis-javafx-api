...
public interface LedgerTransactionDetailsRepository extends JpaRepository<LedgerTransactionDetails, Long> {
    ...
    List<LedgerTransactionDetails> findByLedgerMasterIdAndOutletIdAndTransactionTypeId(long id, Long id1, long l);
/*
...
/*    @Procedure("LEDGER_TRANSACTION_DETAILS_POSTINGS_INSERT")
    void insertIntoLegerTranxDetailsPosting(Long foundation_id, Long principle_id, Long principle_group_id, Long associates_group_id, Long tranx_type_master_id, Long balancing_method_id, Long institute_id, Long branch_id, Long outlet_id, String payment_status, double debit, double credit, LocalDate transactionDate, LocalDate payment_date, Long tranx_id, String transaction_name, String under_prefix, String financial_year, Long created_by, Long ledger_master_id);*/
    ...
    @Query(
            value = "SELECT * FROM `ledger_transaction_details_tbl` where principle_groups_id IN (1,4,5) Or (principle_id IN(9,12) " +
                    ...
                    "AND transaction_type_id!=4 AND transaction_type_id!=6)) OR (principle_groups_id=6 AND principle_id=6 AND transaction_type_id=10)" + " AND outlet_id=?1", nativeQuery = true
    )
    ...
    @Query(
            value = "SELECT * FROM `ledger_transaction_details_tbl` where principle_groups_id IN (1,2,4,5) Or (principle_id IN(9,12) " +
                    "AND (transaction_type_id!=1 AND transaction_type_id!=3 AND transaction_type_id!=2 " +
                    "AND transaction_type_id!=4)) OR (principle_groups_id=6 AND principle_id=6 AND transaction_type_id=10)" + " AND outlet_id=?1 AND branch_id=?2",
            nativeQuery = true
    )
    List<LedgerTransactionDetails> findbygroupIdAndBranch(Long id, Long id1);

    List<LedgerTransactionDetails> findByLedgerMasterIdAndOutletIdAndBranchIdAndTransactionTypeId(long id, Long id1, Long id2, long l);
    ...
    @Query(
            value = "SELECT IFNULL(SUM(closing_bal),0.0),IFNULL(ledger_transaction_details_tbl.principle_id,0) FROM `ledger_transaction_details_tbl` LEFT JOIN ledger_master_tbl ON " +
                    "ledger_transaction_details_tbl.ledger_master_id=ledger_master_tbl.id WHERE ledger_transaction_details_tbl.outlet_id=?1 " +
    ...
    @Query(
            value = "SELECT IFNULL(SUM(closing_bal),0.0),IFNULL(ledger_transaction_details_tbl.principle_id,0) FROM `ledger_transaction_details_tbl` LEFT JOIN ledger_master_tbl ON " +
                    "ledger_transaction_details_tbl.ledger_master_id=ledger_master_tbl.id WHERE ledger_transaction_details_tbl.outlet_id=?1 AND" +
    ...
    @Query(
            value = "SELECT SUM(closing_bal),principle_id  FROM `ledger_transaction_details_tbl` WHERE ledger_transaction_details_tbl.outlet_id=?1 AND ledger_transaction_details_tbl.branch_id=?2 " +
    ...
    @Query(
            value = "SELECT IFNULL(SUM(closing_bal),0),ledger_master_id FROM `ledger_transaction_details_tbl` WHERE ledger_transaction_details_tbl.outlet_id=?1 AND " +
    ...
    @Query(
            value = "SELECT IFNULL(SUM(closing_bal),0),  IFNULL(opening_bal,0) FROM `ledger_transaction_details_tbl` WHERE outlet_id=?1 AND branch_id=?2 status=?3 AND" +
                    "ledger_master_id=?4 AND DATE(transaction_date) BETWEEN ?5 ANd ?6 ;", nativeQuery = true
    )
   Double findByTotalAmountByMonthStartDateAndEndDateAndBranchAndOutletAndStatus(Long id, Long id1, boolean b, Long principle_id, LocalDate startDate, LocalDate endDate);

    @Query(
            value = "SELECT IFNULL(SUM(closing_bal),0) ,IFNULL(opening_bal,0) FROM `ledger_transaction_details_tbl` WHERE outlet_id=?1 AND status=?2 AND " +
                    "ledger_master_id=?3 AND DATE(transaction_date) BETWEEN ?4 ANd ?5 ;", nativeQuery = true
    )
   Double findByTotalAmountByMonthStartDateAndEndDateAndOutletAndStatus(Long id, boolean b, Long principle_id, LocalDate startDate, LocalDate endDate);

    //For Profit And Loss Account Step 3
    @Query(
            value = "SELECT * FROM `ledger_transaction_details_tbl` WHERE outlet_id=?1 AND (branch_id=?2 OR branch_id IS NULL) AND  status=?3 AND " +
                    "ledger_master_id=?4 AND DATE(transaction_date) BETWEEN ?5 ANd ?6 ;", nativeQuery = true
    )
    List<LedgerTransactionDetails> findByIdAndOutletIdAndBranchAndStatusStep3(Long id, Long id1, boolean b, Long ledger_master_id, LocalDate startDate, LocalDate endDate);

    @Query(
            value = "SELECT * FROM `ledger_transaction_details_tbl` WHERE  outlet_id=?1 AND " +
                    " status=?2  AND ledger_master_id=?3 AND transaction_date BETWEEN ?4 AND ?5", nativeQuery = true
    )
    List<LedgerTransactionDetails> findByIdAndOutletIdAndStatusStep3(Long id, boolean b, Long ledger_master_id, LocalDate startDate, LocalDate endDate);
    @Query(
            value = "SELECT ledger_transaction_details_tbl.opening_bal FROM ledger_transaction_details_tbl WHERE outlet_id=?1 AND branch_id=?2 AND status=?3 AND" +
                    " id=?4 AND transaction_date=?5", nativeQuery = true
    )
    Double findByIdAndOutletIdAndBranchIdAndStatuslt(Long id, Long id1, boolean b, Long principle_id, LocalDate startDatep);
    @Query(
            value = "SELECT ledger_transaction_details_tbl.opening_bal FROM ledger_transaction_details_tbl WHERE outlet_id=?1 AND status=?2 AND" +
                    " id=?3 AND transaction_date=?4", nativeQuery = true
    )
    Double findByIdAndOutletIdAndStatusoplt(Long id, boolean b, Long principle_id, LocalDate startDatep);
